...
from attacks.chained_attacks.two_step_chained_attack import ScenarioLoader, AttackEvaluator, BaseInputOutputInterceptor


class LLMInjectionToFunctionChain(Attack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/two_step/test_llm_injections/test_chain_llm_injection_to_function_injection.json"):
    ...
    def execute(self, messages: List[Dict], agent, model_type: str, llm_judge=None) -> Dict:
        ...
        for scenario in progress_bar:
            try:
                ...
                response = llm_function_injector.intercept_conversation(scenario_messages, scenario)
...
class LLMInjectionToResponseChain(Attack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/two_step/test_llm_injections/test_chain_llm_injection_to_response_injection.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    def execute(self, messages: List[Dict], agent, model_type: str, llm_judge=None) -> Dict:
        ...
        for scenario in progress_bar:
            try:
                ...
                response = llm_response_injector.intercept_conversation(scenario_messages, scenario)
                results[scenario.scenario_id] = evaluator.evaluate_response(scenario, response)
            except Exception as e:
                results[scenario.scenario_id] = evaluator.record_failure(e, scenario)

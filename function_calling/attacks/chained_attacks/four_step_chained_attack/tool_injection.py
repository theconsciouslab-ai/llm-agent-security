...
class ToolToLLMToFunctionToResponseChain(Attack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/four_step/test_tool_injections/test_chain_tool_injection_to_llm_to_function_to_response.json"):
    ...
    def execute(self, messages: List[Dict], agent, model_type: str, llm_judge=None) -> Dict:
        ...
        progress_bar = tqdm(
            self.scenarios,
            desc="Testing tool injection -> llm injection -> function injection -> response injection chain",
            unit="scenario",

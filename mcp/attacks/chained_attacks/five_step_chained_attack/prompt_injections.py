...
class MCPPromptInjectionToToolToLLMToFunctionToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/five_step_chained/test_chain_prompt_injection_tool_to_llm_to_function_to_response.json"):
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing prompt injection -> tool injection -> llm injection -> function injection -> response injection",
                unit="scenario",
            ...
            async for scenario in progress_bar:
                try:
                    ...
                    async with tool_interceptor.injection_context(scenario), llm_interceptor.intercept_response(scenario), fun_interceptor.intercept_response(scenario), response_interceptor.intercept_response(scenario):

...
class MCPPromptInjectionToToolToLLMChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_tool_to_llm.json"):
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing prompt injection -> tool injection -> llm injection",
                unit="scenario",
...
class MCPPromptInjectionToToolToFunctionChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_tool_to_function.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing prompt injection -> tool injection -> function injection",
                unit="scenario",
                bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
            )
...
class MCPPromptInjectionToToolToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_tool_to_response.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing prompt injection -> tool injection -> response injection",
                unit="scenario",
                bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
            )
...
class MCPPromptInjectionToLLMToFunctionChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_llm_to_function.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
...
class MCPPromptInjectionToLLMToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_llm_to_result.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        progress_bar = tqdm_asyncio(
            self.scenarios,
                desc="Testing prompt injection -> llm injection -> response injection",
            unit="scenario",
            bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
        )
...
class MCPPromptInjectionToFunctionToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_prompt_injections/test_chain_prompt_injection_to_function_to_response_injection.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        progress_bar = tqdm_asyncio(
            self.scenarios,
                desc="Testing prompt injection -> function injection -> response injection",
            unit="scenario",
            bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
        )
        ...
        async for scenario in progress_bar:
            try:
                ...
                fn_interceptor = FunctionInjectionInterceptor(client)
                response_interceptor = ResponseInjectionInterceptor(client)
                async with fn_interceptor.intercept_response(scenario), response_interceptor.intercept_response(scenario):
                    response = await client.process_query(modified_messages)
                results[scenario.scenario_id] = evaluator.evaluate_response(scenario, response)

...
class MCPToolInjectionToLLMToFunctionChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_tool_injections/test_chain_tool_injection_to_llm_function.json"):
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing tool injection -> llm injection -> function injection",
                unit="scenario",
            ...
            async for scenario in progress_bar:
                try:
                    ...
                    async with tool_interceptor.injection_context(scenario) ,llm_interceptor.intercept_response(scenario), fn_interceptor.intercept_response(scenario):
...
class MCPToolInjectionToLLMToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_tool_injections/test_chain_tool_injection_to_llm_to_response.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing tool injection -> llm injection -> response injection",
                unit="scenario",
                bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
            )
...
class MCPToolInjectionToFunctionToResponseChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/three_step_chained/test_tool_injections/test_chain_tool_injection_to_function_to_response.json"):
        super().__init__(name, description)
        self.scenarios = ScenarioLoader.load_scenarios(scenario_file)
    ...
    async def execute(self,client, messages,llm_judge):
        ...
        try: 
            ...
            progress_bar = tqdm_asyncio(
                self.scenarios,
                desc="Testing tool injection -> function injection -> response injection",
                unit="scenario",
                bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
            )
            ...
            async for scenario in progress_bar:
                try:
                    ...
                    fn_interceptor = FunctionInjectionInterceptor(client)
                    response_interceptor = ResponseInjectionInterceptor(client)
                    async with tool_interceptor.injection_context(scenario), fn_interceptor.intercept_response(scenario), response_interceptor.intercept_response(scenario):
                        response = await client.process_query(test_messages)
                    results[scenario.scenario_id] = evaluator.evaluate_response(scenario, response)

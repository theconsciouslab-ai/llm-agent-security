...
class FunctionInjectionInterceptor:
    ...
    async def modified_call_tool(self, function_name: str, arguments: Dict[str, Any]):
        try:
            ...
            if not original_output.content:  # Handle empty content
            ...
            if self.scenario and self.scenario.malicious_output:
                ...
                if modified_content and len(modified_content) > 0:
                    if isinstance(self.scenario.malicious_output, dict):
                        modified_content[0].text = json.dumps(self.scenario.malicious_output)
                    else:
                        modified_content[0].text = str(self.scenario.malicious_output)
                    original_output.content = modified_content
            return original_output
        except Exception as e:
            print(f"Tool call interception failed: {e}")
            raise

    def patched_create(self, *args, **kwargs):
        response = self.original_create(*args, **kwargs)
        return self.modify_tool_calls(response, self.scenario)
...
class MCPFunctionInjectionChain(MCPAttack):
    def __init__(self, name: str, description: str, scenario_file: str = "./tests/test_chained/one_step_chained/test_chain_function_injection.json"):
